...
# 	menu = Select(options=Location_list,value='DKI Jakarta', title = 'Location')  # drop down menu
# 	bokeh_p=figure(x_axis_label ='Date', y_axis_label = 'Total Active Cases', y_axis_type="linear",x_axis_type="datetime") #creating figure object 
...
# 	date_range_slider = DateRangeSlider(value=(min(df['Date']), max(df['Date'])),start=min(df['Date']), end=max(df['Date']))
